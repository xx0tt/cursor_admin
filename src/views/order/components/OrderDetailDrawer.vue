<template>
  <!-- 订单详情抽屉 -->
  <el-drawer v-model="drawerVisible" title="订单详情" :size="800" :before-close="handleClose">
    <!-- 抽屉内容 -->
    <div v-loading="orderLoading" class="drawer-content">
      ...
      <div class="info-section" v-if="orderDetail">
        <div class="section-header">
          ...
          <el-tag :type="ORDER_STATUS_TYPES[orderDetail.status]" size="default">
          ...
          <el-table :data="orderDetail.equipmentDetails" size="small">
            ...
            <el-table-column prop="status" label="状态" width="80">
              <template #default="{ row }">
                <el-tag :type="row.status === '租赁中' ? 'warning' : 'success'" size="small">
                  {{ row.status }}
                </el-tag>
              </template>
            </el-table-column>
    ...
    <template #footer>
      <div class="drawer-footer">
        ...
        <el-button type="primary" @click="handleViewFull"> 查看完整详情 </el-button>
        <el-button type="primary" @click="handleEdit" v-if="canEdit"> 编辑订单 </el-button>
      </div>
    </template>
  </el-drawer>
</template>
...
// 使用 storeToRefs 确保得到的是 ref
const { currentOrder: orderDetail, orderLoading } = storeToRefs(orderStore)
// 常量直接从 store 读取
...
const canEdit = computed(() => {
  ...
  return [ORDER_STATUS.PENDING, ORDER_STATUS.CONFIRMED].includes(detail.status)
})
