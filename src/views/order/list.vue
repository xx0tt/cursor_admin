  ...
  <div class="order-list-container page-container">
    ...
    <div class="page-header">
      ...
      <div class="header-actions">
        <el-button type="primary" :icon="Plus" @click="handleCreateOrder"> 新建订单 </el-button>
    ...
    <div class="table-section card">
      ...
      >
        ...
        <!-- 订单号 -->
        <el-table-column prop="orderNo" label="订单号" width="140" sortable="custom">
          <template #default="{ row }">
            <el-button type="text" @click="handleViewDetail(row)" class="order-link">
              {{ row.orderNo }}
            </el-button>
          </template>
        </el-table-column>
        ...
        <el-table-column label="设备信息" width="180">
          <template #default="{ row }">
            <div class="equipment-info">
              ...
              <div class="equipment-detail">{{ row.equipmentModel }} × {{ row.quantity }}</div>
            </div>
          </template>
        </el-table-column>
        ...
        <el-table-column prop="status" label="状态" width="100" align="center">
          <template #default="{ row }">
            <el-tag :type="ORDER_STATUS_TYPES[row.status]" size="small">
        ...
        <el-table-column label="操作" width="200" fixed="right">
          <template #default="{ row }">
            <div class="action-buttons">
              ...
              <el-button type="text" size="small" @click="handleEdit(row)" v-if="canEdit(row)">
                编辑
              </el-button>
...
// 解构响应式 ref（保证 orders 为 ref）
const { orders, orderLoading } = storeToRefs(orderStore)

// 非 ref 常量直接从 store 上获取
const { ORDER_STATUS, ORDER_STATUS_LABELS, ORDER_STATUS_TYPES } = orderStore
...
const displayOrders = computed(() => {
  const sourceOrders = Array.isArray(orders.value) ? orders.value : []
  ...
  if (searchForm.equipmentType) {
    filteredOrders = filteredOrders.filter(
      order => order.equipmentType === searchForm.equipmentType
    )
  }
...
const handleDelete = async order => {
  try {
    await ElMessageBox.confirm(`确定要删除订单"${order.orderNo}"吗？此操作不可恢复。`, '确认删除', {
      confirmButtonText: '确定',
      cancelButtonText: '取消',
      type: 'warning'
    })
...
const handleBatchDelete = async () => {
  try {
    ...
    const promises = selectedOrders.value.map(order => orderStore.deleteOrder(order.id))
...
const getStatusActions = order => {
  ...
  switch (order.status) {
    ...
    case ORDER_STATUS.IN_PROGRESS:
      actions.push({ label: '完成订单', value: ORDER_STATUS.COMPLETED })
      break
  }
