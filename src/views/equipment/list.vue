  ...
  <div class="equipment-list-container page-container">
    ...
    <div class="page-header">
      ...
      <div class="header-actions">
        <el-button type="primary" :icon="Plus" @click="handleAddEquipment">
          添加设备
        </el-button>
    ...
    <div class="table-section card">
      ...
      >
        ...
        <el-table-column prop="status" label="状态" width="100" align="center">
          <template #default="{ row }">
            <el-tag
              :type="EQUIPMENT_STATUS_TYPES[row.status]"
              size="small"
            >
        ...
        <el-table-column prop="condition" label="设备状况" width="100">
          <template #default="{ row }">
            <el-tag
              :type="getConditionType(row.condition)"
              size="small"
            >
              {{ row.condition }}
            </el-tag>
...
const equipmentTypes = reactive([
  '挖掘机', '装载机', '推土机', '压路机', '起重机', '搅拌车'
])

// 计算属性
const displayEquipment = computed(() => {
  let filteredEquipment = [...equipmentList.value]
  ...
  if (searchForm.type) {
    filteredEquipment = filteredEquipment.filter(equipment =>
      equipment.type === searchForm.type
    )
  }
  
  if (searchForm.status) {
    filteredEquipment = filteredEquipment.filter(equipment =>
      equipment.status === searchForm.status
    )
  }
